...
async function saveImage(cert:Certificate,side:'front'|'back'){await document.fonts.ready;const canvas=drawCertificate(cert,side);const blob=await new Promise<Blob|null>(resolve=>canvas.toBlob(resolve,'image/png'));if(!blob)throw new Error('导出失败');downloadBlob(blob,`月球漫游纪念证书-${cert.version}-${side==='front'?'正面':'游园记录'}.png`);trackEvent('certificate_export_success',{side,style:cert.style})}
