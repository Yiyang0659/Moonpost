...
保留原来的五种游戏机制，重做为现代月球邮局主题。深灰星空、冷白星尘、少量暖金色点缀，以及悬浮月球和互动玉兔；游戏页面也使用同一视觉规范。
...
src/
  ...
  components/scene/              CosmicBackground 星空 / LunarScene 自转月球 / MoonCourier 坐姿送信循环
...
玉兔使用用户提供的透明坐姿图片，明信片使用可编辑 SVG 代码，月球使用原生 WebGL 球面渲染，接入 NASA CGI Moon Kit 的月面颜色与高程纹理，表现月海、陨石坑与柔和侧光。支持缓慢自动自转、上下左右拖动旋转、四个方向键旋转及 Home 复位；拖动后暂停 3.5 秒再恢复自转，不提供缩放和月面功能标记。手机在月球上滑动旋转月球，在月球外滑动滚动页面。月球有轻微悬浮动画。玉兔坐在轨道上，从右上白点沿前侧轨道移动到左下白点（约 12 秒），转向月面放下信封后立即消失，约 0.65 秒后回到起点循环。轻触玉兔可加速当前配送；减少动态效果时保持静止并提供点击反馈。星尘预先缓存绘制；月球在离开视口或切到后台时暂停；系统开启减少动态效果时关闭自动旋转与悬浮，但仍支持手动旋转。WebGL 或月面纹理不可用时展示静态月球。设计参考本地「宇宙星空前端交互」的星尘层次、细线和留白。public/images/moon-night.png是上版生成插画，目前新版不使用。题材和玩法设计研究见 docs/中秋设计与玩法调研.md；其中明确区分来源事实与本项目设计建议。
