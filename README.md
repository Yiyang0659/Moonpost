...
玉兔与明信片使用可编辑SVG代码，月球使用原生 WebGL 球面渲染，接入 NASA CGI Moon Kit 的月面颜色与高程纹理，表现月海、陨石坑与柔和侧光。支持缓慢自动自转、左右拖动旋转、方向键旋转及 Home 复位；拖动后暂停 3.5 秒再恢复自转，不提供缩放和月面功能标记。手机保留纵向页面滚动。月球和玉兔有轻微悬浮动画。星尘预先缓存绘制；月球在离开视口或切到后台时暂停；系统开启减少动态效果时关闭自动旋转与悬浮，但仍支持手动旋转。WebGL 或月面纹理不可用时展示静态月球。设计参考本地「宇宙星空前端交互」的星尘层次、细线和留白。public/images/moon-night.png是上版生成插画，目前新版不使用。题材和玩法设计研究见 docs/中秋设计与玩法调研.md；其中明确区分来源事实与本项目设计建议。
